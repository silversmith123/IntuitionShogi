...
class CUI:
	...
	def eval(self):
		inputs_koma_b = np.array([self.features['koma_b']])
		inputs_koma_w = np.array([self.features['koma_w']])
		inputs_hand_koma_b = np.array([self.features['hand_koma_b']])
	...
	def forward(self, move):
		self.features = preprocessing.update_features(self.board, move, self.features)

	def back(self):
		self.board.pop()
		self.features = preprocessing.board_to_features(self.board)
	...
	def show_feature(self):
		...
		for type in ['hand_koma_b', 'hand_koma_w']:
			print(type)
			line = ''
			for koma in self.features[type]:
				line += str(int(koma))
			print(line)
